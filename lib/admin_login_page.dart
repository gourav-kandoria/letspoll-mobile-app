...
class admin_login_page extends StatefulWidget {
  @override
  admin_login_page_state createState() => admin_login_page_state();
}

class admin_login_page_state extends State<admin_login_page> {
  String group_name = "";
  String user_name = "";
  String password = "";
  ...
  bool group_name_verifier() {
    ...
    if (on_submit_grp) on_submit_grp = false;
    setState(() {
      err_on_grp_name = null;
    });
    return true;
                        ...
                        group_exists.then((value) {
                          ...
                          if (body['exists'] == true) {
                            ...
                            poll_future.then((value) {
                              ...
                              var user_future =
                                  create_user(client, poll_info['poll_id']);
                              user_future.then((value) {
                                ...
                                make_admin_future.then((value) {
                                  ...
                                  var login_future = login_user(client, user_name, password, poll_info["poll_id"]); 
                                  login_future.then((value) {
                                    Map<String, dynamic> login_info = jsonDecode(value.body); 
                                    //print(login_info['token']);
                                    var store = FlutterSecureStorage();
                                    store.write(key: 'letspole',value: login_info['token']);
  ...
  Future<Response> create_user(Client client, String poll_id) async {
    var data = {
      'poll_id': "${poll_id}",
      'username': user_name,
      'password': password,
  ...
  Future<Response> make_admin(Client client, String poll_id, String user_id,
      String secret_token) async {
        print("Hello World....");
    Response response =
        await client.post('${server_url}/api/auth/make_admin/', body: jsonEncode({
          'poll_id': "${poll_id}",
          'user_id': "${user_id}",
  ...
  Future<Response> login_user (Client client, String u, String p, String pid) async {
    print(u); 
    print(p);
    print(pid);
    Response response = 
        await client.post('${server_url}/api/auth/login/',body: jsonEncode( {
          'username': u,
          'password': p,
          'poll_id': pid,
        }
